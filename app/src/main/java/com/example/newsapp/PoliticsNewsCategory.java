...
public class PoliticsNewsCategory extends Category {

    //private List<Articles> articlesArrayList;
    private TaskCompletedListener listener;
    private final String category = "Politics";

    public PoliticsNewsCategory(TaskCompletedListener listener){
    ...
    protected Document doInBackground(String... urls) {
        ...
        Document doc = null;
        try {
            doc = Jsoup.connect("https://www.cnbc.com/politics/").get();
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    protected void onPostExecute(Document doc) {
        if (doc != null) {
            ...
            Elements articles = doc.select("div:has(a.Card-title) a.Card-title");
                        ...
                        public void onImageSearchComplete(String imageUrl) {
                            ...
                            for (Articles article : MainActivity.articlesArrayList) {
                                if (article.getTitle().equals(title)) {
                                    ...
                                    count++;
                                    //break;
                                    MainActivity.politicsArticles.add(article);
