...
public class SportsNewsCategory extends Category {
    ...
    protected Document doInBackground(String... urls) {
        ...
        Document doc = null;
        try {
            doc = Jsoup.connect("https://www.thenews.com.pk/latest/category/sports").get();
        }
        catch (IOException e)
        {
            e.printStackTrace();
    ...
    protected void onPostExecute(Document doc) {
        if (doc != null) {
            ...
            Elements articles = doc.select("div.latest-right h2 a.open-section");
//            Elements links = doc.select("h3 a[href]");
            ...
            for (int i = 0; i < Math.min(10, articles.size()); i++) {
                ...
                if (!newsUrl.startsWith("http")) {
                    newsUrl = "https://www.thenews.com.pk" + newsUrl;
                }
                        ...
                        public void onImageSearchComplete(String imageUrl) {
                            ...
                            for (Articles article : MainActivity.articlesArrayList) {
                                if (article.getTitle().equals(title)) {
                                    ...
                                    count++;
                                    //break;
                                    MainActivity.sportsArticles.add(article);
